...
def GetTrainingList():
    user_id = get_jwt_identity()
    list = []
    amount_of_trainings = set(
        iter.training_id
        for iter in Trainings.query.filter_by(user_id=user_id).all())
    for training_id in amount_of_trainings:
        exercises_list = []
        for single_exercise in (Trainings.query
                                         .filter_by(training_id=training_id)
                                         .all()):
            exercises_list.append({
                'exercise_name': single_exercise.exercise_name,
                'exercise_weight': single_exercise.exercise_weight})
        list.append({"date": single_exercise.date,
                     "comment": single_exercise.comment,
                     "exercises": exercises_list,
                     "weight": single_exercise.weight})
...
def DeleteSelectedExercises():
    ...
    for exercise in exercises:
        db.session.delete(Exercises.query
